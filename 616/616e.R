...
xy <- lonlat2map(grid$lon, grid$lat, projection=p)
#xy <- read.table("xy.dat", col.names=c("x", "y"), header=FALSE)
d <- data.frame(lon=grid$lon, lat=grid$lat, x=round(xy$x), y=round(xy$y))
write.table(d, file="616e_results.dat", row.names=FALSE)
if (!interactive()) png("616e.png", unit="in", width=6, height=3, res=150, pointsize=7)
