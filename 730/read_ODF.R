	...
	for( idxLine in 1:(data_lines_index+1) ) {
	  ...
	  #for the last line).
		...
		if( length(headIndex) > 0 || idxLine >= data_lines_index) {
			...
			pramIndex <- grep(paste("^",val[1],"$",sep=""), headObject[[curParmeterName]][,1], useBytes=TRUE)
...
if(length(grep("SYTM_01",pram_names, useBytes=TRUE)) > 0) {
...
extract_val <- function(line) {
	...
	equIndex <- grep("=", charArray, useBytes=TRUE)[1]
...
convert_number <- function(sVal) {
	...
	for( i in 1:length(val)) {
		if( grepl("([0-9]+D(\\+|-)[0-9][0-9])", val[i], useBytes=TRUE)) {
