...
cm1 <- colormap(p, breaks=seq(zlim[1], zlim[2], 0.5), debug=3)
...
par(mar=omar)
cm2 <- colormap(p, zlim=zlim, breaks=5, debug=3)
...
par(mar=omar)
cm3 <- colormap(p, breaks=seq(zlim[1], zlim[2], length.out=11), zclip=TRUE, debug=3)
stopifnot(!any(is.na(cm3$zcol)))
drawPalette(colormap=cm3)
plot(t, p, type='p', pch=21, cex=1.5, bg=cm3$zcol) 
abline(h=zline)
mtext("3: colormap(z, breaks=seq(38.5, 40.5, length.out=11), zclip=TRUE)", side=3, cex=3/4)
...
mtext("looks ??  to DK", side=3, line=-1, font=2, cex=3/4)
mtext("looks BAD to CR", side=3, line=-2, font=2, cex=3/4)
