...
xfile <- c("x1.dat", "x2.dat")
pxxfile <- c("pxx1.dat", "pxx2.dat")
...
for (ifile in seq_along(xfile)) {
    ...
    if (ifile == 1) {
        ...
        startE <- scan(text='1  36  71 106 141 176 211 246', quiet=TRUE)
        endE <- scan(text='70 105 140 175 210 245 280 315', quiet=TRUE)
    }
    ...
    if (!interactive())
        png(paste("1359a", ifile, ".png", sep=""),
            width=7, height=5, unit="in", res=150, pointsize=9)
