...
for (lon_0 in -60) {
    ...
    if (TRUE) {
        ...
        plot(lon, lat, xlim=c(-62,-56), ylim=c(-85,-63), type='o', pch=20, cex=2/3)
        points(lon[LOOK], lat[LOOK], pch=20, col=2:3)
        ...
        plot(mod$xo, mod$yo, xlim=c(-62,-56), ylim=c(-85,-63), type='o', pch=20, cex=2/3)
        points(mod$xo[mod$insideo], mod$yo[mod$insideo],pch=20, col='red')
        polygon(mod$xo, mod$yo, col='lightgray')
        abline(v=lon_0, col='red', lwd=1/2, lty='dotted')
        points(mod$xo, mod$yo, col='red')
